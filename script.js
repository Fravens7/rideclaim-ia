...
// ====================================================================
// EXPORTS FOR EXTERNAL MODULES (schedule-ui.js)
// ====================================================================
window.fileResults = fileResults;
window.updateResultsTable = updateResultsTable;
window.calculateTotalSpent = calculateTotalSpent;
window.calculateActiveDays = calculateActiveDays;
window.updateSummaryCards = updateSummaryCards;
window.setResultsView = setResultsView;
window.renderGroupedResults = renderGroupedResults;
// We also need to keep fileResults in sync if it's reassigned, but it's a const reference to an array (wait, it's let fileResults = []).
// If fileResults is reassigned (e.g. filter), window.fileResults will be stale.
// Better to use a getter/setter or just be careful.
// In updateTripResultsFromAI (old version), we did fileResults = fileResults.filter...
// In the NEW merge version, we modify in place, so the reference stays valid.
// BUT, if we ever reassign fileResults, we must update window.fileResults.
// For now, let's just expose it.
